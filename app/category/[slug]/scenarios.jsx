...
export default function CategoryScenarios() {
  ...
  const fetchScenarios = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/categories/${slug}/scenarios`);
  ...
  if (isLoading) {
    return (
      <View style={tw`flex-1 bg-slate-50 justify-center items-center`}>
        ...
        <Text style={tw`text-xs text-slate-400 mt-2`}>Memuat daftar pedoman...</Text>
      </View>
    );
  }
        ...
        {scenarios && scenarios.length > 0 ? (
          scenarios.map((scen) => (
            ...
            >
              ...
              <View style={tw`flex-1`}>
                <Text style={tw`text-sm font-extrabold text-slate-800 mb-0.5`} numberOfLines={1}>
                  {scen.title}
                </Text>
                ...
                <View style={tw`flex-row flex-wrap gap-1`}>
                  {scen.tags?.map((tag) => (
                    <View key={tag.id} style={tw`bg-slate-50 border border-slate-100 px-2 py-0.5 rounded`}>
          ...
          <View style={tw`bg-white border border-dashed border-slate-200 p-8 rounded-2xl items-center justify-center`}>
